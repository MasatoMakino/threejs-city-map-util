...
/** biome-ignore-all lint/correctness/noUnusedVariables: 外部ライブラリ */
/** biome-ignore-all lint/correctness/noPrecisionLoss: 外部ライブラリ */
...
export function latlon2xy(
  ...
  const alpha = [
    0.0, 8.377318247285465e-4, 7.608527848379248e-7, 1.1976455002315586e-9,
    2.4291502606542468e-12, 5.750164384091974e-15,
  ];
  ...
  const t = Math.sinh(
    Math.atanh(Math.sin(lat)) - coef0 * Math.atanh(coef0 * Math.sin(lat)),
  );
  ...
  const scaleFactor =
    (aOverline / longerRadius) *
    Math.sqrt(
      ((sigma * sigma + tau * tau) / (t * t + cosDiffLon * cosDiffLon)) *
        (1 + coefM * coefM),
    );
...
export function xy2latlon(x: number, y: number, zone: number[]) {
  ...
  /** β(i)　(i=1,2,...5) */
  const beta = [
    0.0, 8.377321681620316e-4, 5.905870211016955e-8, 1.6734826761541112e-10,
    2.1648237311010893e-13, 3.79409187887551e-16,
  ];
  /** δ(i) (i=1,2,...6) */
  const delta = [
    0.0, 0.003356551485604312, 6.571873263127177e-6, 1.7646404372866207e-8,
    5.3877538900094696e-11, 1.7640075159133883e-13, 6.056074055207582e-16,
  ];
  ...
  const lonInRadian =
    toRadian(originLon) + Math.atan(Math.sinh(etaDash) / Math.cos(xiDash));
...
export function xyzone(orgLat: number, orgLon: number) {
  ...
  const largeA = [
    1.0000007049454078, -0.0025188297041239312, 2.6435429493240994e-6,
...
export function xyzonejapan(sysno: number) {
  ...
  const sOverline = [
    0, 3652382.768270788, 3652382.768270788, 3985144.116029223,
    3652382.768270788, 3985144.116029223, 3985144.116029223, 3985144.116029223,
    3985144.116029223, 3985144.116029223, 4429086.077333566, 4873334.987359202,
    4873334.987359202, 4873334.987359202, 2876546.889061122, 2876546.889061122,
    2876546.889061122, 2876546.889061122, 2212145.0174775715, 2876546.889061122,
  ];
  /** 参照論文の変数「Aの上付線」の平面直角座標系1～19毎の原点に対応した定数。latlon2xyとxy2latlonで共通 */
  const aOverline = [
    0, 6366812.400856472, 6366812.400856472, 6366812.400856472,
    6366812.400856472, 6366812.400856472, 6366812.400856472, 6366812.400856472,
    6366812.400856472, 6366812.400856472, 6366812.400856472, 6366812.400856472,
    6366812.400856472, 6366812.400856472, 6366812.400856472, 6366812.400856472,
    6366812.400856472, 6366812.400856472, 6366812.400856472, 6366812.400856472,
  ];
